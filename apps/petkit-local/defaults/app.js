...
// The Ingenic models (T5/T6/T7/D4H/D4SH) do NOT answer bare JSON. They speak
// the same 8-byte-header frame the rest of this codebase already reads
// (devices/ble.py `_ble_unframe`, payload at offset 8), on a JSON channel keyed
// by byte[3] = 0x46, with a CRC before the trailer:
//
//   FA FC FD 46 | type | seq | len_le16 | json_utf8 | crc16_le | FB
//
// type is 0x18 phone->device, 0x13 device->phone; len is the JSON length as a
// 16-bit little-endian field; crc is CRC-16/CCITT-FALSE over the JSON bytes,
// little-endian. The whole frame goes out in ONE writeWithoutResponse — bare
// JSON, a mid-frame split, or a write *request* all end in the device saying
// nothing back. Verified against a live T6 (upstream issue #9); the D4H is the
// same family.
...
// Wrap one JSON object in the provisioning frame.
...
// Pull the JSON object out of one inbound frame (a DataView from Web
// Bluetooth), or null if it is not a provisioning frame.
function pkParse(view) {
  const u = new Uint8Array(view.buffer);
  if (u.length < 11) return null;
  if (u[0] !== 0xfa || u[1] !== 0xfc || u[2] !== 0xfd || u[3] !== 0x46) return null;
  const len = u[6] | (u[7] << 8);
  try {
    return JSON.parse(new TextDecoder().decode(u.slice(8, 8 + len)));
  } catch (e) {
    return null;
  }
}
...
async function provisionPetkit(service, cfg) {
  ...
  let acked = false; // saw a 151 {state:1}
  ...
  tx.addEventListener('characteristicvaluechanged', ev => {
    ...
    if (!msg) {
      ...
      plog('device (unframed): ' + [...raw].map(x => x.toString(16).padStart(2, '0')).join(' '));
      return;
    }
    replies[msg.key] = msg.payload || {};
    plog('device: key ' + msg.key + ' ' + JSON.stringify(msg.payload || {}));
    if (msg.key === 151 && (msg.payload || {}).state === 1) acked = true;
  });

  // One ATT write per frame, WITHOUT response — the firmware NAKs a write
  // request and will not answer a split frame.
  const write = rx.writeValueWithoutResponse
  ...
  // 110: ask who it is. Its reply proves the frame format is right before any
  // credentials go out, and gives the Devices tab the real device id.
  plog('asking the device who it is (key 110)…');
  await send({ key: 110, payload: {} });
  await new Promise(r => setTimeout(r, 800));

  // 151: Wi-Fi + where to phone home.
  plog('sending Wi-Fi credentials and server address (key 151)…');
  await send({ key: 151, payload: cfg.payload });

  // Wait up to ~5s for the {state:1} ack.
  for (let i = 0; i < 20 && !acked; i++) await new Promise(r => setTimeout(r, 250));

  // Poll join status (key 112). 7 = reached server, 10 = joined; either is done.
  if (acked) {
  ...
  // "Heard" now means the device answered the framed protocol at all — the thing
  // that never once happened with bare JSON.
  return acked || Object.keys(replies).length > 0;
}
